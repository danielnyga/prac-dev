...
// similar to HighlightAll, but returns generated html code instead of setting it directly
dp.sh.HighlightGivenHTML = function(name, html, showGutter /* optional */, showControls /* optional */, collapseAll /* optional */, firstLine /* optional */, showColumns /* optional */)
{
	function FindValue()
	{
		var a = arguments;
	...
	function FindTagsByName(list, name, tagName, html)
	{
		var s = html;
		var wrapperDiv = document.createElement('div');
		wrapperDiv.innerHTML = s;

		for(var i = 0; i < wrapperDiv.childNodes.length; i++) {
			if(wrapperDiv.childNodes[i].getAttribute('name') == name) {
				list.push(wrapperDiv.childNodes[i]);
			}
		}
	}
	...
	// for some reason IE doesn't find <pre/> by name, however it does see them just fine by tag name... 
	FindTagsByName(elements, name, 'pre', html);
	FindTagsByName(elements, name, 'textarea', html);
