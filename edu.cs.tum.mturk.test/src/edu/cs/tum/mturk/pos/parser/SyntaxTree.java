...
public class SyntaxTree {
	...
	//		System.out.println( "(" + tree.getElement().getType() + ", " + tree.getElement().getName() + ")" );
		...
		for ( Iterator<SyntaxTree> i = tree.getChildren().iterator(); i.hasNext(); ) {
	...
	private void buildSentenceInternal( StringBuffer str ) {
		...
		if ( getChildren().size() > 0 ) {
			for ( Iterator<SyntaxTree> i = getChildren().iterator(); i.hasNext(); ) {
	...
	public static ArrayList<SyntaxElement> leaves(SyntaxTree tree) 
	{

		ArrayList<SyntaxElement> leaves = new ArrayList<SyntaxElement>();
		...
		if ( tree.getChildren().size() > 0 ) 
		{
			...
			{
				SyntaxTree t = (SyntaxTree) i.next();
				leaves.addAll(leaves(t));
			}
		}else{
			if (tree.getElement() != null && ! tree.getElement().getName().equals( "" ))
				leaves.add(tree.getElement());
		}
